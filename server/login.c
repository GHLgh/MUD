...
pthread_mutex_t new_user_lock = PTHREAD_MUTEX_INITIALIZER;
...
int map_userinfo(char* filename){
  ...
  while((len = getline(&buffer, &size, fd))!=-1){
    ...
    user_num++;
    user_array = realloc(user_array, sizeof(user_info)*user_num);
...
int match_password(int index, char* new_user){
  ...
  for(; i < 3; i++){
    ...
    if(index == -1){
      pthread_mutex_lock(&new_user_lock);
      ...
      pthread_mutex_unlock(&new_user_lock);
      return user_num-1;
    }
